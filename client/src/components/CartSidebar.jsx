                        ...
                        {cartProducts.map(ci =>(
                            <div key={ci.productId} className='flex gap-3 p-3 rounded-lg border bg-card'>
                                ...
                                <div className='flex-1 min-w-0'>
                                    ...
                                    <p className='text-sm font-bold text-primary'>₹{ci.product.price.toFixed(2)}</p>
                                    <div className='flex items-center gap-2 mt-1'>
                                        <Button variant="outline" size="icon" className="h-7 w-7" onClick={()=>ci.quantity>1 && updateQty(ci.productId,ci.quantity-1)}>
                                            <Minus className="h-3 w-3" />
                                        </Button>
                                        <span className='text-sm font-medium w-6 text-center'>{ci.quantity}</span>
                                        <Button variant="outline" size="icon" className="h-7 w-7" onClick={()=>ci.quantity<ci.product.stock && updateQty(ci.productId,ci.quantity+1)}>
                                            <Plus className="h-3 w-3" />
                                        </Button>
                                    </div>
                                </div>
                                <div className='flex flex-col items-end justify-between'>
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={()=> removeItem(ci.productId)}>
                                        <Trash2 className="h-4 w-4" />
                                    </Button>
                                    <span className='text-sm font-bold'>₹{(ci.product.price * ci.quantity).toFixed(2)}</span>
                                </div>
                                </div>
                        ))}
                      </div>
